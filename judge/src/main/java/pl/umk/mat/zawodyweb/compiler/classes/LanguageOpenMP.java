 ...
 */
public class LanguageOpenMP implements CompilerInterface {

    public static final org.apache.log4j.Logger logger = Logger.getLogger(LanguageOpenMP.class);
    ...
    public TestOutput runTest(String path, TestInput input) {
        ...
        if (!System.getProperty("os.name").toLowerCase().matches("(?s).*windows.*")) {
            ...
            command = Arrays.asList("bash", "-c", "ulimit -v " + (input.getMemoryLimit() * 1024) + " -t " + (5 + input.getTimeLimit() / 1000) + 
                    " && export OMP_NUM_THREADS=" + numThreads + " && '" + path + "'");
        } else {
...
//                output.setResult(ResultsStatusEnum.UNDEF.getCode());
//                return output;
            } catch (Exception ex) {
        ...
        try {
            ...
            try {
                List<String> command = new ArrayList<String>(
                        Arrays.asList("gcc", "-O2", "-static", "-o", compilefile, codefile, "-lm", "-fopenmp"));
