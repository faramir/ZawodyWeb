...
public class LanguagePAS implements CompilerInterface {
    ...
    public TestOutput runTest(String path, TestInput input) {
        ...
        if (compileResult!=CheckerErrors.UNDEF) {output.setResult(compileResult);
        ...
        while(time>new Date().getTime()) {
            ...
            p.destroy();
            p.waitFor();
            if (p.exitValue()!=0) {output.setResult(CheckerErrors.RE);
    ...
    public String compile(byte[] code) {
        ...
        if (compileResult!=CheckerErrors.UNDEF) return new String();
        try {
            ...
            p.destroy();
            p.waitFor();
            if (p.exitValue()!=0) {compileResult = CheckerErrors.CE;
            int i = 1;
            while ((line = input.readLine()) != null) {
                if (i>4) compileDesc = compileDesc + line+"\n";
                i++;
            }
            input.close();
            }
